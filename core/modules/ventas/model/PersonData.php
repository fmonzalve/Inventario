...
class PersonData {
	...
	public function add_client(){
		$sql = "insert into person (name,lastname,address1,address2,email1,phone1,kind,created_at) ";
		$sql .= "value (\"$this->name\",\"$this->lastname\",\"$this->address1\",\"$this->address2\",\"$this->email1\",\"$this->phone1\",1,$this->created_at)";
		Executor::doit($sql);
	}
...
// partiendo de que ya tenemos creado un objecto PersonData previamente utilizamos el contexto
	...
	public function update_client(){
		$sql = "update ".self::$tablename." set name=\"$this->name\",email1=\"$this->email1\",address1=\"$this->address1\",address2=\"$this->address2\",lastname=\"$this->lastname\",phone1=\"$this->phone1\" where id=$this->id";
		Executor::doit($sql);
	}
	...
	public static function getLike($q){
		$sql = "select * from ".self::$tablename." where name like '%$q%' or lastname like '%$q%'";
		$query = Executor::doit($sql);
		$array = array();
		$cnt = 0;
		while($r = $query[0]->fetch_array()){
    	...
    	public static function getLikeF($q){
		$sql = "select * from ".self::$tablename." where kind='1' and (name like '%$q%' or lastname like '%$q%')";
		$query = Executor::doit($sql);
		$array = array();
		$cnt = 0;
		while($r = $query[0]->fetch_array()){
    	...
    	public static function getLikeCli($q){
		$sql = "select * from ".self::$tablename." where kind=1 and (name like '%$q%' or lastname like '%$q%')";
		$query = Executor::doit($sql);
		$array = array();
		$cnt = 0;
