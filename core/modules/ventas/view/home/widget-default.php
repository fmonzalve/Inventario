...
<?php endif;?>
        ...
        <div class="alert alert-info"><h3>Se mostrarán alertas cuando el stock de un producto sea bajo.</h3></div>
...
foreach($curr_products as $product):
	...
	<tr class="<?php if($q==0){ echo "danger"; }else if($q<=$product->inventary_min/2){ echo "danger"; } else if($q<=$product->inventary_min){ echo "warning"; } ?>">
        ...
        <td>
            <form action="index.php?view=re&product=<?php echo "$product->name"; ?>" method="POST">
                <input type="submit" class="btn btn-success" value="Reabastecer">
            </form>
        </td>
	</tr>
<?php endif;?>
