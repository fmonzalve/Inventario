            ...
            <form>
                <div class="row">
                    <div class="col-md-6">
                        ...
                        <select name="opcion">
                            <option value="C" <?php if($_GET["opcion"]=="C"){echo "selected";} ?> >Cualquiera</option>
                            <option value="V" <?php if($_GET["opcion"]=="V"){echo "selected";} ?> >Activo Variable</option>
                            <option value="F" <?php if($_GET["opcion"]=="F"){echo "selected";} ?> >Activo Fijo</option>                        
                        </select>
                    </div>
                </div>
                <br>
...
if($_GET["opcion"]=="V"){
    $products = ProductData::getLikeV($_GET["product"]);
}elseif ($_GET["opcion"]=="F") {
    $products = ProductData::getLikeF($_GET["product"]);
}else{
...
$q= OperationData::getQYesF($product->id);
	...
	<tr class="<?php if($q<=$product->inventary_min){ echo "danger"; }?>">
		...
		<input type="number" class="form-control" required name="q" placeholder="Cantidad de producto ..." min="1"></td>
        
        <td>
            <input type="number" class="form-control" required name="precio" placeholder="Precio de producto ..." min="0"></td>
		<td style="width:100px;">
		
        <button type="submit" class="btn btn-primary"><i class="glyphicon glyphicon-shopping-cart"></i> Agregar</button>
		</td>
	</tr>
	</form>
...
<?php if(!isset($_GET["bus_fun"])){ ?>

<p><b>Buscar Funcionario:</b></p>
<form method=GET action='index.php?view=sell'>
    <div class="row">
        <div class="col-md-6">
            <input type="hidden" name="view" value="sell">
...
<form method="post" class="form-horizontal" id="processsell" action="index.php?view=processsell">
    <?php if(isset($_GET["bus_fun"])){ 
            ...
            $clients = PersonData::getLikeF($_GET["bus_fun"]);
            ...
            if(count($clients)>0){
                ...
                <div class="form-group">
                    ...
                    <div class="checkbox">
                        ...
                        <button class="btn btn-lg btn-primary"><i class="glyphicon glyphicon-usd"></i><i class="glyphicon glyphicon-usd"></i> Finalizar Entrega</button>
                        </label>
                    </div>
                    </div>
                </div>
...
<div class="form-group">
    ...
    <div class="col-lg-10">
      <input type="text" name="money" required class="form-control" id="money" placeholder="Efectivo" value=0>
    </div>
  </div>
